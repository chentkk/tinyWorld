...
function modifier_unstable_concoction_lua_counter:OnDestroy()
    ...
    if not self.bIsUse then
        ...
        local enemies = FindUnitsInRadius(self:GetParent():GetTeam(), self:GetParent():GetAbsOrigin(), nil, self:GetAbility():GetSpecialValueFor("radius"), abilityThrow:GetAbilityTargetTeam(), abilityThrow:GetAbilityTargetType(), abilityThrow:GetAbilityTargetFlags(), 0, false)
        for _, target in pairs(enemies) do
            ...
            target:AddNewModifier(self:GetCaster(), self:GetAbility(), 'modifier_stunned', {duration = self:GetAbility():GetSpecialValueFor('max_stun')})
        end
