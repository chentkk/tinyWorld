LinkLuaModifier( "modifier_ability_wave_of_terror", "heroes/vengeful_spirit/wave_of_terror" ,LUA_MODIFIER_MOTION_NONE )
...
function ability_wave_of_terror:OnProjectileHit(Target, Location)
    if Target ~= nil and not Target:IsInvulnerable() then
        ...
        Target:AddNewModifier(self:GetCaster(), self, "modifier_ability_wave_of_terror", {duration=armor_reduction_duration})
    end
    return false
end

function ability_wave_of_terror:OnProjectileThink(vLocation)
    local caster = self:GetCaster()
...
modifier_ability_wave_of_terror = class({
...
function modifier_ability_wave_of_terror:OnCreated()
    self.armor_reduction = self:GetAbility():GetSpecialValueFor("armor_reduction")
end

function modifier_ability_wave_of_terror:GetModifierPhysicalArmorBonus() return self.armor_reduction end