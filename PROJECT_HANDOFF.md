...
- **关键依赖字段不允许用 `or fallback` 静默吞 nil**（例如 `runtime`、`cell`、`space`、`source`、
  `config` 等）。这类字段在当前上下文中必须存在：缺失必须用 `assert` 显式失败或输出错误日志，
  不允许"保护行"把错误状态悄悄转成默认值，从而隐藏真问题。
- 可选配置（带明确默认值的业务数值）可以写默认值，但必须当场注释默认语义；
  可选回调用显式 `if` 判断，不允许把"未配置"和"配置为假值"混为一谈。
- **禁止无意义的布尔规范化写法**：不得写 `not not x`、`x and true or false`、`not x and true or false`
  这类只为"压成布尔"的表达式。需要布尔契约时直接写 `== true` / `== false`，或对回调返回值
  显式 `assert(type(v) == "boolean", ...)`；回调/字段必须返回明确的 boolean，而不是把 nil 当 false。
