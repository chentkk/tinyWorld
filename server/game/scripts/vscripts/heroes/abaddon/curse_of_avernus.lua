...
function modifier_ability_curse_of_avernus_lua:OnAttackLanded(data)
    ...
    if modifierManager.hasModifier(data.target, "modifier_ability_curse_of_avernus_lua_debuff") then return end

    local ability = self:GetAbility()
    data.modifierManager.addModifier(target, "modifier_ability_curse_of_avernus_lua_debuff", ability, {
        duration = tonumber(ability.data.slowDuration) or 2,
    })
    modifierManager.addModifier(self:GetCaster(), "modifier_ability_curse_of_avernus_lua_buff", ability, {
        duration = tonumber(ability.data.slowDuration) or 2,
    })
end
