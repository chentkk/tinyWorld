...
-- 提供 add/remove/set/get 及视图同步 op。
...
function Container:isViewOpened() return self.isView end
function Container:getViewId() return self.viewId end
...
function Container:setViewProp(name, value)
    ...
    prop[name] = value
    self.viewProps = prop
    if self.isView then self.dirty[#self.dirty + 1] = { type = "view", data = { [name] = value } } end
    if self.host.onContainerPersist then self.host:onContainerPersist(self) end
end
...
function Container:add(data)
    ...
    local child = { id = id, props = {}, records = {} }
    local childSchema = self.def.childSchema
    for _, f in ipairs(childSchema.fields) do
        local v = data[f.name]
        if v ~= nil then child.props[f.name] = childSchema:coerce(f.name, v)
        elseif f.default ~= nil then child.props[f.name] = f.default end
    end
    ...
    if self.isView then self.dirty[#self.dirty + 1] = { type = "add", id = id, data = self:childFullData(child) } end
    if self.host.onContainerPersist then self.host:onContainerPersist(self) end
    return true
end

function Container:remove(id)
    if not self.children[id] then return false end
    self.children[id] = nil
    if self.isView then self.dirty[#self.dirty + 1] = { type = "remove", id = id } end
    if self.host.onContainerPersist then self.host:onContainerPersist(self) end
    return true
end

function Container:get(id) return self.children[id] end
...
function Container:has(id) return self.children[id] ~= nil end
...
function Container:setChildProp(id, name, value)
    ...
    if self.isView then self.dirty[#self.dirty + 1] = { type = "set", id = id, data = { [name] = value } } end
    if self.host.onContainerPersist then self.host:onContainerPersist(self) end
    return true
end
...
function Container:childFullData(child)
    ...
    for name, rec in pairs(child.records) do out[name] = rec:dump() end
    return out
end
...
function Container:dump()
    local out = {}
    for id, child in pairs(self.children) do out[#out + 1] = self:childFullData(child) end
    return out
end

function Container:load(list)
    for _, data in ipairs(list or {}) do self:add(data) end
end
