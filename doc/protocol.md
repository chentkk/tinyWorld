...
| `record`  | 表格（Record）同步 |
| `view`    | 容器/视图（View）同步 |
| `batch`   | 批量信封（服务器把同一 tick 的多条消息合并为一帧） |

### 2.2 批量信封 batch（服务器 → 客户端）

服务器每个 tick 会把**发给同一玩家的所有同步消息**合并成一帧 `batch`，减少跨服务发送与编码。
客户端收到后需**按顺序展开逐条处理**，语义与逐条收到完全一致：

```json
{ "t": "batch", "n": "msgs", "d": { "msgs": [ { "t": "prop", "n": "props", "d": { ... } }, { "t": "record", ... } ] } }
```

- `d.msgs` 是消息数组，元素就是上面各类普通消息（`prop`/`record`/`view`/`object`/`RPC` 等）。
- 不在 batch 内的消息（如 `object add` 初进视野、`AUTH`、`ACCOUNT`）仍是单条，客户端需同时支持两种形态。
- `message.log` 里服务器会把 batch **逐条展开记录**，因此日志仍然是逐条的消息，便于联调。
...
- `props` 是全量属性快照，包含 `dir`（朝向，见下）。

#### 朝向 dir

实体的 `dir` 是**连续朝向角**（弧度，与 `math.cos/sin` 一致）：
...
移动是**任意方向自由移动**，`dir` 随移动方向连续变化（例如向右下移动时 `dir ≈ pi/4`）。
"4 方向"只是客户端的美术表现方式：客户端把连续 `dir` 量化为 4 个方向后选择对应 sprite。
量化规则见 `tinyworld/core/direction.lua`（`quantize`），正好落在 45° 分界线时按
RIGHT → DOWN → LEFT → UP 的顺序取靠前者。

> 说明：投掷物/技能的**瞄准角度**是独立的 `angle` 属性（同为弧度），与实体的 `dir` 区分。
...
- 只包含发生变化且需要同步的属性。
- `seq` 是服务器声明的 **`sync = "self"` 属性**：只在发给**自己**的属性包里出现，周围玩家/ghost 收不到。
- 它表示服务器已处理的最后一次 `onRequestMove` 序号，用于客户端 Reconciliation；只有移动时 `seq` 才变化，因此纯属性变更（如 hp）的包不会带 `seq`。
